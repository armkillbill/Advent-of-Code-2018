...
func main() {
	...
	mostGuardTotalSleep, mostSleepGuardID, minuteOfMostSleep := 0, 0, 0
	for guardID, data := range table {
		minuteOfMaxSleep, guardMaxSleep, guardTotalSleep := 0, 0, 0
		for minute, time := range data {
			guardTotalSleep += time
			if time > guardMaxSleep {
				minuteOfMaxSleep = minute
				guardMaxSleep = time
			}
		}
		if guardTotalSleep > mostGuardTotalSleep {
			mostGuardTotalSleep = guardTotalSleep
